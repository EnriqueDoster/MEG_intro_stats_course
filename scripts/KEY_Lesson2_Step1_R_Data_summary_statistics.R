...
# For question 1
sum(sample_metadata$X16S_Raw_paired_reads)

# For question 2
sum(sample_metadata$Shotgun_raw_reads)
...
mean(updated_sample_metadata$percent_host_removed)
...
# For question 4, you can just run the newly aggregated phyloseq object.
phylum.ps

# We can also export the taxa file using tax_table(), convert it to a data frame with as.data.frame().
# Then we can access the phylum column with the "$" from that object and use unique()
unique(as.data.frame(tax_table(microbiome.ps))$phylum)
...
# For question 5, you have the same options as for question 4
species.ps
unique(as.data.frame(tax_table(microbiome.ps))$species)
...
sum(sample_sums(species.ps))  /  sum(sample_sums(microbiome.ps)) * 100
...
# For question 8
species_kraken.ps <- tax_glom(kraken_microbiome.ps, "species")
sum(sample_sums(species_kraken.ps))  /  sum(sample_sums(kraken_microbiome.ps)) * 100
...
# For question 10
sum(sample_sums(mechanism_amr.ps))  /  sum(sample_sums(amr.ps)) * 100
...
combined_diversity_values %>%
  group_by(SeqType) %>%
  summarize(max_observed = max(Observed), max_shannon = max(Shannon))

# For question 12
combined_diversity_values %>%
  group_by( Group, SeqType) %>%
  summarize(mean_observed = mean(Observed), mean_shannon = mean(Shannon))
